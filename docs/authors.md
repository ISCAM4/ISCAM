# Authors and Citation

## Authors

- **Beth Chance**. Maintainer, author, copyright holder.

- **Visruth Srimath Kandali**. Author.
  [](https://orcid.org/0009-0005-9097-0688)

## Citation

Source:
[`DESCRIPTION`](https://github.com/ISCAM4/ISCAM/blob/master/DESCRIPTION)

Chance B, Srimath Kandali V (2026). *ISCAM: Companion to the Book
"Investigating Statistical Concepts, Applications, and Methods"*. R
package version 1.1.1.9000, <https://iscam4.github.io/ISCAM/>.

    @Manual{,
      title = {ISCAM: Companion to the Book "Investigating Statistical Concepts, Applications, and Methods"},
      author = {Beth Chance and Visruth {Srimath Kandali}},
      year = {2026},
      note = {R package version 1.1.1.9000},
      url = {https://iscam4.github.io/ISCAM/},
    }
