...
k is the number of successes of interest (must be integer), n and prob are the number of trials and success probability \n lower.tail is a Boolean for finding the probability above (FALSE) or below (TRUE) the inputted value (inclusive)"
  ...
  if (!lower.tail) {
    ...
    showprob <- format(this.prob, digits = 4)
    lines(k:n, dbinom(k:n, size = n, prob), col = "red", type = "h", lwd = 2)
    text((maxx + n * prob) * 9 / 16, myy, labels = paste("P(X \u2265", k, ")\n =", showprob), pos = 1, col = "red")
    cat("Probability", k, "and above =", this.prob, "\n")
  }
  newtitle <- substitute(paste("Binomial (", n == x1, ", ", pi == x2, ")", ), list(x1 = n, x2 = prob))
  title(newtitle)
  mtext(side = 1, line = 2, "Number of Successes")
  mtext(side = 2, line = 2, "Probability")
...
binomtest <- function(observed, n, hypothesized = NULL, alternative, conf.level = NULL) {
  ...
  if (!is.null(hypothesized)) {
    ...
    newtitle <- substitute(paste("Binomial (", n == x1, ", ", pi == x2, ")", ), list(x1 = n, x2 = hypothesized))
    title(newtitle)
    mtext(side = 1, line = 2, "Number of Successes")
    mtext(side = 2, line = 2, "Probability")
...
invbinom <- function(alpha, n, prob, lower.tail) {
  ...
  if (lower.tail) {
    ...
    lines(0:answer, dbinom(0:answer, size = n, prob), col = "red", type = "h", lwd = 2)
    text(x = minx, y = myy, labels = paste("P(X \u2264", answer, ")=", actualprob, "\nP(X \u2264", answer + 1, ")=", format(pbinom(answer + 1, n, prob, lower.tail), digits = 4)), col = "red", pos = 4)
    # text(answer, dbinom(answer, size=n, prob), labels=actualprob, pos=2, col="red")
    text(answer, dbinom(answer, size = n, prob), labels = paste("X=", answer), col = "red", pos = 3)
  ...
  if (!lower.tail) {
    ...
    lines(answer:n, dbinom(answer:n, size = n, prob), col = "red", type = "h", lwd = 2)
    myx <- (maxx + n * prob) / 2
    ...
    # text(answer, dbinom(answer, size=n, prob), labels=actualprob, pos=2, col="red")
    text(answer, dbinom(answer, n, prob), labels = paste("X=", answer), col = "red", pos = 3)
